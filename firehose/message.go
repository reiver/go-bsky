...
func (receiver Message) Decode(header *MessageHeader, payload *MessagePayload) error {
...

	{
		...
		if nil != err {
			return erorr.Errorf("bsky: problem decoding cbor header: %w", err)
		}
	}
	...
	var headersize int
	{
		...
		if nil != err {
			return erorr.Errorf("bsky: problem encoding cbor header (after decoding it, to get length): %w", err)
		}
...

	{
		...
		if nil != err {
			return erorr.Errorf("bsky: problem decoding cbor payload: %w", err)
		}
	}
